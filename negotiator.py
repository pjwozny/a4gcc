...
class BilateralNegotiatorWithOnlyTariff():
  ...
  def check_bilateral_negotiation(self, region_id):
        ...
        accepted = self.rice.global_state["proposal_decisions"]["value"][self.rice.timestep, region_id, :]
        #proposal decision of :, region_id would be all it has proposed, any presence of 1 is an acceptance by another state
        accepted_by_other = self.rice.global_state["proposal_decisions"]["value"][self.rice.timestep, :, region_id]
        ...
        for region_id in range(self.rice.num_regions):
            ...
            if self.rice.negotiation_on:
                ...
                for other_region_id in range(self.rice.num_regions):
                    ...
                    if (other_region_id == region_id) or (not self.check_bilateral_negotiation(region_id)):
                        
                        #make no change to tariff policy
                        regional_tariff_mask = np.array([1 for _ in range(self.rice.num_discrete_action_levels)])
                        ...
                        region_tariff_rate = int(round(self.rice.num_discrete_action_levels - self.rice.global_state["minimum_mitigation_rate_all_regions"]["value"][
                        self.rice.timestep, other_region_id
                    ]))
                        ...
                        regional_tariff_mask = np.array(
                            ...
                            + [
                                1
                                for _ in range(
                                    self.rice.num_discrete_action_levels - region_tariff_rate
                                )
                            ]
                        )
                ...
                mask_start_tariff = sum(self.rice.savings_action_nvec
                                    + self.rice.mitigation_rate_action_nvec
                                    + self.rice.export_action_nvec
                                    + self.rice.import_actions_nvec)
